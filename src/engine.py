...
#url='https://www.baidu.com/s?ie=UTF-8&wd=python'#入口
...
while queue:
    ...
    #爬取网页内容
    try:
        response=requests.get(url, headers)#request.urlopen(url)
        content=response.text#response.read().decode('gb18030')
    except:
        continue		
